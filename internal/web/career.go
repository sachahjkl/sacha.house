...
func careerFR() CareerProfile {
	return CareerProfile{
		...
		Projects: []CareerProject{
			{Name: "sacha.house", URL: "https://gitlab.com/sachahjkl/sacha.house", Summary: "Site personnel et moteur de publication en Go, avec composants templ, navigation Datastar, authentification WebAuthn et déploiement Nix.", Description: "Je développe mon site personnel et son moteur de publication en Go. Le serveur utilise templ pour les composants, Datastar pour la navigation progressive et WebAuthn pour authentifier l'accès à l'interface d'administration. Le build, les livraisons et le déploiement reposent sur Nix.", Stack: "Go, templ, Datastar, Tailwind CSS, WebAuthn, Nix"},
			{Name: "nixconfig", URL: "https://gitlab.com/sachahjkl/nixconfig", Summary: "Infrastructure NixOS déclarative utilisée pour reconstruire plusieurs machines et services à partir de modules partagés.", Description: "Je maintiens une infrastructure NixOS déclarative afin de rendre mes systèmes reproductibles et de limiter les écarts de configuration entre les machines. Les mêmes modules permettent de reconstruire quatre configurations, dont trois machines physiques et un environnement WSL. Le dépôt contient 123 modules flake-parts, 94 modules NixOS et 24 entrées de flake ; il configure 31 hôtes virtuels et 20 conteneurs.", Stack: "Nix, NixOS, flake-parts, systemd, conteneurs"},
			{Name: "albumator", URL: "https://gitlab.com/sachahjkl/albumator", Description: "J'ai développé cette application de dépôt, de classement et de consultation d'images avec Svelte 5 et SvelteKit.", Stack: "Svelte 5, SvelteKit, Tailwind CSS"},
			{Name: "clockin.sacha.house", URL: "https://gitlab.com/sachahjkl/clockin.sacha.house", Description: "Application personnelle de pointage conçue autour d'une interaction minimale : un bouton enregistre le temps et des statistiques restituent l'activité. L'identité ne requiert ni courriel ni mot de passe ; un identifiant court généré par le serveur suffit pour retrouver les données. L'application réunit une SPA Angular 22, une API Fastify, Drizzle et SQLite dans un service NixOS reproductible.", Stack: "Angular 22, Fastify, TypeScript, Drizzle ORM, SQLite, NixOS"},
			...
			{Name: "Activité GitLab", URL: "https://gitlab.com/sachahjkl", Description: "Mon profil rassemble 49 projets personnels, neuf projets scolaires répartis dans huit groupes et deux contributions externes, en plus de quelques forks. Je suis l'auteur principal de presque tous les projets personnels.", Stack: "Projets personnels, travaux scolaires et contributions open source"},
		},
		Catalog: []CareerGroup{
...
func careerEN() CareerProfile {
	...
	profile.Experiences = []CareerExperience{
		{
			...
			Details: []CareerDetail{
				...
				{Title: "Profiles and permissions", Text: "A major contribution involved the shared design of a cross-cutting system, now used in production instead of historical checks scattered across individual features. Permission calculation combines groups from Azure DevOps and Active Directory, database attributes tied to the user, and session information. These inputs are matched against hierarchical profile declarations, their permissions, and complementary dynamic rules. The result drives both conditional rendering in Angular and access control in the .NET APIs."},
	...
	profile.Projects = []CareerProject{
		{Name: "sacha.house", URL: "https://gitlab.com/sachahjkl/sacha.house", Summary: "Personal website and publishing engine in Go, with templ components, Datastar navigation, WebAuthn authentication, and Nix deployment.", Description: "I develop my personal website and publishing engine in Go. The server uses templ for components, Datastar for progressive navigation, and WebAuthn to authenticate access to the administration interface. Nix handles builds, releases, and deployment.", Stack: "Go, templ, Datastar, Tailwind CSS, WebAuthn, Nix"},
		{Name: "nixconfig", URL: "https://gitlab.com/sachahjkl/nixconfig", Summary: "Declarative NixOS infrastructure used to rebuild several machines and services from shared modules.", Description: "I maintain declarative NixOS infrastructure to make my systems reproducible and limit configuration drift between machines. Shared modules rebuild four configurations, including three physical computers and one WSL environment. The repository contains 123 flake-parts modules, 94 NixOS modules, and 24 flake inputs; it configures 31 virtual hosts and 20 containers.", Stack: "Nix, NixOS, flake-parts, systemd, containers"},
		{Name: "albumator", URL: "https://gitlab.com/sachahjkl/albumator", Description: "I developed this image upload, organization, and browsing application with Svelte 5 and SvelteKit.", Stack: "Svelte 5, SvelteKit, Tailwind CSS"},
		{Name: "clockin.sacha.house", URL: "https://gitlab.com/sachahjkl/clockin.sacha.house", Description: "Personal clock-in application designed around minimal interaction: one button records time and statistics summarize activity. Identity requires neither email nor password; a short server-generated identifier is enough to retrieve the data. The application packages an Angular 22 SPA, a Fastify API, Drizzle, and SQLite as a reproducible NixOS service.", Stack: "Angular 22, Fastify, TypeScript, Drizzle ORM, SQLite, NixOS"},
		...
		{Name: "GitLab activity", URL: "https://gitlab.com/sachahjkl", Description: "My profile contains 49 personal projects, nine academic projects across eight groups, and two external contributions, plus a few forks. I am the primary author of almost every personal project.", Stack: "Personal projects, academic work, and open-source contributions"},
	}
